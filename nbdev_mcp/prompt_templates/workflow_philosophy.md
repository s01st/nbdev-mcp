...
2. ✅ **ALWAYS run `{nbdev_export_cmd}`** after editing notebooks
...
4. Run `{nbdev_export_cmd}` to regenerate the .py file
...
5. `{nbdev_export_cmd}` → Regenerates {lib}/core.py
...
- `{nbdev_export_cmd}` - Generate .py files from notebooks
- `{nbdev_test_cmd}` - Run tests in notebooks (quick checks)
...
- Always export after editing notebooks with `{nbdev_export_cmd}`.
